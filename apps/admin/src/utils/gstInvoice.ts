...
export interface GSTInvoiceItem {
  ...
  bagSize?: string;
  quantity: number;
  unitPrice: number; // inclusive or calculated taxable
...
export function buildGSTInvoiceFromOrder(order: {
  ...
  // 5% GST inclusive breakdown (HSN 0901: CGST 2.5% + SGST 2.5%)
  const taxableTotal = +(total / 1.05).toFixed(2);
  const totalGst = +(total - taxableTotal).toFixed(2);
  const cgst = +(totalGst / 2).toFixed(2);
  const sgst = +(totalGst - cgst).toFixed(2); // balance pennies
...
export function renderGSTInvoiceHTML(inv: GSTInvoiceData): string {
