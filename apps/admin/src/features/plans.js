...
function parseEntitlements(raw) {
    ...
    for (const line of raw.split('\n').map((l) => l.trim()).filter(Boolean)) {
        ...
        if (!match)
            return { ok: false, error: `Cannot read "${line}" — use CODE x UNITS, e.g. CONSULT_15MIN x 2` };
        const code = match[1].toUpperCase();
        if (!ENTITLEMENT_CODES.includes(code))
            return { ok: false, error: `Unknown entitlement code "${code}"` };
        value.push({ code, units: Number(match[2]) });
    }
    return { ok: true, value };
}
function entitlementsToText(list) {
...
export function initAdminPlans(portal) {
    void portal;
  ...
  `;
    const formHost = body.querySelector('#plan-form-host');
    const tableBody = body.querySelector('#plans-table-body');
    let plans = [];
    function bindForm(plan) {
        formHost.innerHTML = formHtml(plan);
        const form = formHost.querySelector('#plan-form');
        const errorEl = formHost.querySelector('#plan-form-error');
        ...
        form.addEventListener('submit', async (ev) => {
            ...
            if (!plan)
                payload.slug = String(data.get('slug') || '').trim();
            const res = plan
            ...
            if (!res.success) {
                errorEl.textContent = res.error || 'Could not save the plan';
                return;
            }
            bindForm(null);
            await loadPlans();
    ...
    async function loadPlans() {
        const res = await adminFetch('/api/subscriptions/admin/plans');
        ...
        tableBody.querySelectorAll('[data-retire-plan]').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                if (!confirm(`Retire "${plan.name}"? It disappears from the storefront. Existing members keep their plan and their perks.`))
                    return;
                const res = await adminFetch(`/api/subscriptions/admin/plans/${encodeURIComponent(plan.id)}`, { method: 'DELETE' });
                if (!res.success) {
                    alert(res.error || 'Could not retire the plan');
                    return;
                }
                await loadPlans();
            });
    ...
    async function loadEntitlements() {
        const el = body.querySelector('#plan-entitlements-body');
        const res = await adminFetch('/api/subscriptions/admin/entitlements');
    ...
    async function loadSubscribers() {
        const el = body.querySelector('#plan-subscribers-body');
        const res = await adminFetch('/api/subscriptions/admin/subscribers');
