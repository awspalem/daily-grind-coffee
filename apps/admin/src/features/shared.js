...
export function mountAdminPanel(panelId, title, headerActionHtml = '') {
 ...
 */
export function registerAdminNavItem(tab, label, panelId) {
    const list = document.querySelector('.nav-item-btn')?.closest('ul');
    if (!list || list.querySelector(`[data-tab="${tab}"]`))
        return;
