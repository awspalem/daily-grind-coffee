...
export function initAdminExperiences(portal) {
    void portal;
  ...
  `;
    const expBody = body.querySelector('#exp-table-body');
    const slotBody = body.querySelector('#slot-table-body');
    const rosterHost = body.querySelector('#roster-host');
    const blackoutBody = body.querySelector('#blackout-table-body');
    let experiences = [];
    async function loadExperiences() {
        const res = await adminFetch('/api/experiences/admin/experiences');
        ...
        expBody.querySelectorAll('[data-exp-status]').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                if (!res.success) {
                    alert(res.error || 'Could not change the status');
                    return;
                }
        ...
        const pickers = [
            body.querySelector('#slot-experience'),
            body.querySelector('#blackout-experience'),
        ];
        const optionsHtml = experiences
            .map((e) => `<option value="${esc(e.id)}">${esc(e.name)} (${esc(e.mode)})</option>`)
            .join('');
        if (pickers[0])
            pickers[0].innerHTML = optionsHtml;
        if (pickers[1])
            pickers[1].innerHTML = `<option value="">All experiences</option>${optionsHtml}`;
    }
    async function loadSlots() {
        const res = await adminFetch('/api/experiences/admin/slots?upcoming=1');
        ...
        slotBody.querySelectorAll('[data-roster]').forEach((btn) => {
            btn.addEventListener('click', () => void loadRoster(btn.dataset.roster));
        });
        slotBody.querySelectorAll('[data-promote]').forEach((btn) => {
            btn.addEventListener('click', async () => {
                const res = await adminFetch(`/api/experiences/admin/slots/${encodeURIComponent(btn.dataset.promote)}/promote-waitlist`, { method: 'POST' });
                alert(res.promotedBookingId ? 'Offered the seat to the next person on the waitlist.' : 'Nobody is waiting for this slot.');
                await loadSlots();
            });
        });
        slotBody.querySelectorAll('[data-cancel-slot]').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                if (!confirm('Cancel this slot? Everyone booked on it is cancelled, refunded, and told by email.'))
                    return;
                const res = await adminFetch(`/api/experiences/admin/slots/${encodeURIComponent(btn.dataset.cancelSlot)}`, { method: 'DELETE' });
                if (!res.success) {
                    alert(res.error || 'Could not cancel the slot');
                    return;
                }
    ...
    async function loadRoster(slotId) {
        const res = await adminFetch(`/api/experiences/admin/slots/${encodeURIComponent(slotId)}/roster`);
        if (!res.success) {
            rosterHost.innerHTML = `<p style="color:var(--danger,#c0392b);">${esc(res.error || 'Could not load the roster')}</p>`;
            return;
        }
    ...
    `;
        rosterHost.querySelectorAll('[data-attend]').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                if (!res.success) {
                    alert(res.error || 'Could not mark attendance');
                    return;
                }
    ...
    async function loadBlackouts() {
        ...
        blackoutBody.querySelectorAll('[data-del-blackout]').forEach((btn) => {
            btn.addEventListener('click', async () => {
    ...
    body.querySelector('#slot-form').addEventListener('submit', async (ev) => {
        ev.preventDefault();
        const form = ev.target;
        const errorEl = body.querySelector('#slot-form-error');
        errorEl.textContent = '';
        const data = new FormData(form);
        const startsAt = localInputToIso(String(data.get('startsAt') || ''));
        if (!startsAt) {
            errorEl.textContent = 'A start time is required';
            return;
        }
        ...
        if (!res.success) {
            errorEl.textContent = res.error || 'Could not create the slot';
            return;
        }
        form.reset();
        await loadSlots();
    });
    body.querySelector('#blackout-form').addEventListener('submit', async (ev) => {
        ev.preventDefault();
        const form = ev.target;
        const errorEl = body.querySelector('#blackout-error');
        errorEl.textContent = '';
        const data = new FormData(form);
        const startsAt = localInputToIso(String(data.get('startsAt') || ''));
        const endsAt = localInputToIso(String(data.get('endsAt') || ''));
        if (!startsAt || !endsAt) {
            errorEl.textContent = 'A blackout needs a start and an end';
            return;
        }
        const res = await adminFetch('/api/experiences/admin/blackouts', {
            method: 'POST',
        ...
        if (!res.success) {
            errorEl.textContent = res.error || 'Could not add the blackout';
            return;
        }
        form.reset();
        await loadBlackouts();
