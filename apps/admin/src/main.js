...
async function loadAiBriefing() {
    ...
    if (!box)
        return;
    try {
        const data = await apiRequest('/api/admin/ai-summary');
...
async function loadInventory() {
    try {
        ...
        const select = document.getElementById('adjust-variant');
        if (select) {
...
async function loadMovements() {
    try {
        const data = await apiRequest('/api/admin/movements');
...
async function loadFunnel() {
    try {
        const data = await apiRequest('/api/analytics/funnel');
        const f = data.funnel;
...
async function loadQuotas() {
    try {
        const data = await apiRequest('/api/admin/quotas');
        const r = data.report;
...
function getStatusBadgeColor(status) {
    if (status === 'PAID' || status === 'DELIVERED')
        return 'success';
    if (status === 'ROASTING' || status === 'PACKED' || status === 'SHIPPED')
        return 'warning';
...
window.refundOrder = async (orderId) => {
    ...
    if (!reason)
        return;
    try {
        const res = await apiRequest(`/api/admin/orders/${orderId}/refund`, {
            method: 'POST',
...
window.quickAdjustStock = (variantId) => {
    const select = document.getElementById('adjust-variant');
    if (select)
...
function setupModals() {
    ...
    document.getElementById('btn-trigger-backup')?.addEventListener('click', async () => {
        ...
        btn.disabled = true;
        try {
            const res = await apiRequest('/api/admin/backup', {
                method: 'POST',
            });
    ...
    document.getElementById('adjust-stock-form')?.addEventListener('submit', async (e) => {
        e.preventDefault();
        const variant_id = document.getElementById('adjust-variant').value;
        const movement_type = document.getElementById('adjust-type').value;
        const quantity_delta = Number(document.getElementById('adjust-qty').value);
