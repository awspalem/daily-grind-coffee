...
class AdminPortal {
    ...
    setupBatchLogging() {
        ...
        const load = async () => {
            const data = await this.adminFetch('/api/admin/roast-batches');
            render(data.batches || []);
        };
        load();
        form.addEventListener('submit', async (e) => {
            ...
            const profileInput = document.getElementById('batch-profile');
            const result = await this.adminFetch('/api/admin/roast-batch', {
                method: 'POST',
                body: JSON.stringify({
    ...
    setupCouponsManager() {
      ...
      `).join('');
        ...
        const load = async () => {
            const data = await this.adminFetch('/api/admin/coupons');
            render(data.coupons || []);
        };
        load();
        document.getElementById('btn-add-coupon')?.addEventListener('click', async () => {
            ...
            if (!discount)
                return;
            const result = await this.adminFetch('/api/admin/coupons', {
                method: 'POST',
