...
class InvestorPortal {
    ...
    // Modeler State
    modelRaiseInr = 5000000; // ₹50 Lakhs ($60k)
    modelPreMoneyInr = 50000000; // ₹5.00 Crores ($600k)
    modelTargetSubscribers = 1200;
    modelProjectedArrInr = 12000000; // ₹1.20 Crores ($144k)
    ...
    formatMoney(inrValue, isCompact = false) {
    ...
    updateAllCurrencyDisplays() {
        ...
        if (heroArr)
            heroArr.textContent = this.formatMoney(2214000, true);
        if (heroMrr)
            heroMrr.textContent = this.formatMoney(184500, true);
        ...
        if (metricArr)
            metricArr.textContent = this.formatMoney(2214000, true);
        if (metricMrr)
            metricMrr.textContent = this.formatMoney(184500, false);
        if (metricAov)
            metricAov.textContent = this.formatMoney(850, false);
        if (metricLtv)
            metricLtv.textContent = this.formatMoney(4850, false);
        if (metricProfit)
            metricProfit.textContent = this.formatMoney(164.09, false);
    ...
    updateScorecardGauge() {
        ...
        if (scoreFraction)
            scoreFraction.textContent = `${verifiedCount} of ${total} Milestones Verified (${pct}%)`;
        if (fillCircle) {
    ...
    // ==========================================
    setupScenarioModeler() {
        const sliderRaise = document.getElementById('slider-raise');
        const sliderPre = document.getElementById('slider-pre');
        const sliderSubs = document.getElementById('slider-subs');
        const sliderArr = document.getElementById('slider-arr');
        const handleInput = () => {
            if (sliderRaise)
                this.modelRaiseInr = parseInt(sliderRaise.value, 10);
            if (sliderPre)
                this.modelPreMoneyInr = parseInt(sliderPre.value, 10);
            if (sliderSubs)
                this.modelTargetSubscribers = parseInt(sliderSubs.value, 10);
            if (sliderArr)
                this.modelProjectedArrInr = parseInt(sliderArr.value, 10);
            this.recomputeModelerOutputs();
        };
    ...
    syncSliders() {
        ...
        const sliderArr = document.getElementById('slider-arr');
        if (sliderRaise)
            sliderRaise.value = this.modelRaiseInr.toString();
        if (sliderPre)
            sliderPre.value = this.modelPreMoneyInr.toString();
        if (sliderSubs)
            sliderSubs.value = this.modelTargetSubscribers.toString();
        if (sliderArr)
            sliderArr.value = this.modelProjectedArrInr.toString();
    }
    recomputeModelerOutputs() {
        // Update Slider Pills
        ...
        if (pillRaise)
            pillRaise.textContent = this.formatMoney(this.modelRaiseInr, true);
        if (pillPre)
            pillPre.textContent = this.formatMoney(this.modelPreMoneyInr, true);
        if (pillSubs)
            pillSubs.textContent = `${this.modelTargetSubscribers.toLocaleString()} Subs`;
        if (pillArr)
            pillArr.textContent = this.formatMoney(this.modelProjectedArrInr, true);
        ...
        if (outEquity)
            outEquity.textContent = `${investorEquityPct.toFixed(2)}%`;
        if (outPost)
            outPost.textContent = this.formatMoney(postMoneyInr, true);
        if (outCapex)
            outCapex.textContent = this.formatMoney(capexAllocationInr, true);
        if (outRunway)
            outRunway.textContent = `${runwayMonths} Months`;
        if (outIrr)
            outIrr.textContent = `${annualizedIRR}% (${moic.toFixed(1)}x MOIC)`;
        if (outBeans)
            outBeans.textContent = `${monthlyBeansKg.toLocaleString()} kg/mo`;
    }
    // ==========================================
