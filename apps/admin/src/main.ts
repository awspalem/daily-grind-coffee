...
class AdminPortal {
  ...
  private setupSubscriptionsManager() {
    const tbody = document.getElementById('subscriptions-table-body');
    if (!tbody) return;

    const statusBadgeClass: Record<string, string> = { ACTIVE: 'paid', PAUSED: 'shipped', CANCELLED: 'low-stock', PAST_DUE: 'low-stock' };

    const render = (subs: any[]) => {
      if (subs.length === 0) {
    ...
    const load = async () => {
      const data = await this.adminFetch<{ success: boolean; subscriptions: any[] }>('/api/admin/subscriptions');
      render(data.subscriptions || []);
    };
    load();
  }

  private setupReviewsManager() {
    ...
    const load = async () => {
      const data = await this.adminFetch<{ success: boolean; reviews: any[] }>('/api/admin/reviews');
      render(data.reviews || []);
    };
    load();
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      this.triggerHaptic();
      const result = await this.adminFetch<{ success: boolean; error?: string }>(`/api/admin/reviews/${reviewId}`, { method: 'DELETE' });
      if (result.success) {
        await load();
      } else {
  ...
  // ==========================================================================
  private setupProductCatalogManager() {
    const tbody = document.getElementById('catalog-table-body');
    const form = document.getElementById('product-add-form') as HTMLFormElement | null;
    const categorySelect = document.getElementById('product-category') as HTMLSelectElement | null;
    const imageFileInput = document.getElementById('product-image-file') as HTMLInputElement | null;
    const imageUrlInput = document.getElementById('product-image-url') as HTMLInputElement | null;
    const imageStatus = document.getElementById('product-image-status');
    const imagePreview = document.getElementById('product-image-preview') as HTMLImageElement | null;
    if (!tbody || !form || !categorySelect || !imageFileInput || !imageUrlInput) return;

    const loadCategories = async () => {
      const data = await this.adminFetch<{ success: boolean; categories: any[] }>('/api/categories');
      categorySelect.innerHTML = (data.categories || []).map((cat) =>
        `<option value="${cat.id}">${this.escapeHtml(cat.name)}</option>`
      ).join('');
    };

    const renderVariantRow = (product: any, variant: any) => `
    ...
    const render = (products: any[]) => {
      tbody.innerHTML = products.map((p) => `
        ...
        ${(p.variants || []).map((v: any) => renderVariantRow(p, v)).join('')}
        ${renderAddVariantRow(p.id)}
      `).join('');
    };

    const load = async () => {
      const data = await this.adminFetch<{ success: boolean; products: any[] }>('/api/admin/products');
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      if (productToggle) {
        ...
        const result = await this.adminFetch<{ success: boolean; error?: string }>(`/api/admin/products/${productToggle.dataset.productToggle}`, {
          method: 'PATCH',
      ...
      if (variantToggle) {
        ...
        const result = await this.adminFetch<{ success: boolean; error?: string }>(`/api/admin/variants/${variantToggle.dataset.variantToggle}/status`, {
          method: 'PATCH',
          body: JSON.stringify({ is_active: nextActive }),
        });
        if (result.success) await load();
      ...
      if (addVariantBtn) {
        ...
        const result = await this.adminFetch<{ success: boolean; error?: string }>(`/api/admin/products/${addVariantBtn.dataset.addVariant}/variants`, {
          method: 'POST',
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const result = await this.adminFetch<{ success: boolean; error?: string }>('/api/admin/products', {
        method: 'POST',
