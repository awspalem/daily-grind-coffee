 ...
 */
export function hasRedeemIntent(): boolean {
  try {
...
function setRedeemIntent(on: boolean): void {
  try {
    if (on) localStorage.setItem(REDEEM_INTENT_KEY, '1');
 ...
 */
export async function redeemPointsForSubtotal(subtotalCents: number): Promise<number> {
  if (!isSignedIn() || !hasRedeemIntent()) return 0;
  const res = await apiFetch<{ preview?: { eligible: boolean; max_points: number } }>(
    '/api/loyalty/redeem/preview',
    { method: 'POST', json: { subtotal_cents: subtotalCents } }
  );
  if (!res.success || !res.preview?.eligible) return 0;
...
function renderSummary(host: HTMLElement, summary: Summary): void {
  ...
  host.querySelector<HTMLInputElement>('#loyalty-redeem-toggle')
    ?.addEventListener('change', (ev) => setRedeemIntent((ev.target as HTMLInputElement).checked));
}

function entryHtml(entry: StatementEntry): string {
...
async function loadStatement(host: HTMLElement, offset: number): Promise<void> {
  const list = host.querySelector<HTMLElement>('#loyalty-statement');
  const more = host.querySelector<HTMLButtonElement>('#loyalty-more');
  if (!list) return;

  const res = await apiFetch<{ entries?: StatementEntry[] }>(
    `/api/loyalty/statement?limit=${PAGE_SIZE}&offset=${offset}`
  );
...
export function initLoyalty(app: any): void {
  void app;
  injectStyles();
  ...
  void (async () => {
    if (!isSignedIn()) {
      const cfg = await apiFetch<{ config?: Record<string, any> }>('/api/loyalty/config');
      renderSignedOut(section, cfg.success ? cfg.config || null : null);
      return;
    }

    const res = await apiFetch<{ summary?: Summary }>('/api/loyalty/summary');
