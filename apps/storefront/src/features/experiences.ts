...
const state = {
  ...
  bookings: [] as Booking[],
  balances: [] as Balance[],
  openSlotsFor: null as string | null,
  busy: false,
};
...
function fmtRange(booking: { starts_at: string; ends_at: string; is_multi_day: number }): string {
...
function creditsFor(code: string | null): number {
  if (!code) return 0;
...
function durationLabel(exp: Experience): string {
  if (exp.is_multi_day) return 'Multi-day';
  if (!exp.duration_minutes) return '';
  return exp.duration_minutes >= 60
    ? `${Math.round((exp.duration_minutes / 60) * 10) / 10} hr`
    : `${exp.duration_minutes} min`;
}

function experienceCard(exp: Experience): string {
...
function slotList(exp: Experience): string {
  ...
  // above is escaped.
  return rows.replace(
    /&lt;span style="color: var\(--accent-terracotta\); font-weight: 600;"&gt;Full — join the waitlist&lt;\/span&gt;/g,
    '<span style="color: var(--accent-terracotta); font-weight: 600;">Full — join the waitlist</span>'
  );
}
...
function bookingRow(b: Booking): string {
  ...
  if (needsAction) actions.push(`<button class="btn-primary" style="padding:0.5rem 1.1rem; font-size:0.88rem;" data-bk-confirm="${esc(b.id)}">Confirm</button>`);
...
function icsUrl(b: Booking): string {
  const base = (window as any).__API_BASE__ || 'https://api.dailyroast.in';
...
function wire(section: HTMLElement): void {
  section.querySelectorAll<HTMLButtonElement>('[data-exp-toggle]').forEach((btn) => {
  ...
  section.querySelectorAll<HTMLButtonElement>('[data-slot-book]').forEach((btn) => {
    btn.addEventListener('click', () => book(btn.getAttribute('data-exp')!, btn.getAttribute('data-slot-book')!));
  });

  section.querySelectorAll<HTMLButtonElement>('[data-bk-confirm]').forEach((btn) => {
    btn.addEventListener('click', () => confirmBooking(btn.getAttribute('data-bk-confirm')!));
  });
  section.querySelectorAll<HTMLButtonElement>('[data-bk-cancel]').forEach((btn) => {
    btn.addEventListener('click', () => cancelBooking(btn.getAttribute('data-bk-cancel')!));
  });
  section.querySelectorAll<HTMLButtonElement>('[data-bk-reschedule]').forEach((btn) => {
    btn.addEventListener('click', () => reschedule(btn.getAttribute('data-bk-reschedule')!));
  });
}

async function book(experienceId: string, slotId: string): Promise<void> {
  if (!isSignedIn()) {
  ...
  const res = await apiFetch<{ booking: Booking; waitlisted: boolean }>('/api/experiences/bookings', {
    method: 'POST',
    json: { slotId, partySize, dietaryNotes, accessibilityNotes, idempotencyKey },
  });
  state.busy = false;

  if (!res.success) { toast(res.error || 'Could not hold that place.', 'error'); return; }
...
async function confirmBooking(bookingId: string): Promise<void> {
  if (state.busy) return;
  state.busy = true;
  const res = await apiFetch<{ checkoutUrl?: string; fundingSource?: string }>(
    `/api/experiences/bookings/${bookingId}/confirm`,
    { method: 'POST', json: {} }
  );
  state.busy = false;

  if (!res.success) { toast(res.error || 'Could not confirm that booking.', 'error'); return; }
...
async function cancelBooking(bookingId: string): Promise<void> {
  ...
  if (!window.confirm(`Cancel this booking?${policy}`)) return;

  state.busy = true;
  const res = await apiFetch<{ entitlementRestored?: boolean; refundStatus?: string }>(
    `/api/experiences/bookings/${bookingId}/cancel`,
    { method: 'POST', json: {} }
  );
  state.busy = false;

  if (!res.success) { toast(res.error || 'Could not cancel that booking.', 'error'); return; }
...
async function reschedule(bookingId: string): Promise<void> {
  ...
  const res = await apiFetch<{ slots: Slot[]; experience: Experience }>(
    `/api/experiences/catalog/${encodeURIComponent(booking.experience_id)}`
  );
  const options = (res.slots || []).filter((s) => s.id !== booking.slot_id && !s.is_full);
  if (!options.length) { toast('There are no other open dates right now.', 'error'); return; }
  ...
  if (!choice) { toast('That was not one of the options.', 'error'); return; }
  ...
  if (!moved.success) { toast(moved.error || 'Could not move that booking.', 'error'); return; }
 ...
 */
async function handleReturnFromStripe(): Promise<void> {
...
async function refresh(): Promise<void> {
  const catalog = await apiFetch<{ experiences: Experience[] }>('/api/experiences');
  state.experiences = catalog.success ? (catalog.experiences || []) : [];

  if (isSignedIn()) {
    const [bookings, balances] = await Promise.all([
      apiFetch<{ bookings: Booking[] }>('/api/experiences/bookings'),
      apiFetch<{ balances: Balance[] }>('/api/experiences/me/entitlements'),
    ]);
...
export function initExperiences(app: any): void {
  void app;
