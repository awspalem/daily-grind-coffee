 ...
 */
import { apiFetch, esc, formatCents, isSignedIn, mountFeatureSection, registerNavPill, } from './shared';
...
function renderSignedOut() {
    ...
    ${panel(`<p style="color:var(--text-muted); text-align:center; margin:0;">
...
function renderStats(p) {
    ...
    ${tiles
        .map((t) => `<div style="background:var(--bg-secondary); border-radius:var(--radius-md); padding:1rem 1.1rem;">
...
function renderDistribution(title, rows, transform) {
...
function renderTasteGraph(p) {
    const usual = [];
    if (p.favourite_grind)
        usual.push(`Ground for ${titleise(p.favourite_grind)}`);
    if (p.typical_weight_grams)
        usual.push(`${weightLabel(p.typical_weight_grams)} bags`);
    if (p.top_roast_level)
        usual.push(`${titleise(p.top_roast_level)} roast`);
    if (p.avg_review_rating)
        usual.push(`You rate us ${p.avg_review_rating}/5 across ${p.review_count} review${p.review_count === 1 ? '' : 's'}`);
    return panel(`
...
function renderRecommendations() {
    if (state.recommendations.length === 0)
        return '';
    const due = state.daysUntilReorder !== null && state.daysUntilReorder <= 7
        ? `<p style="color:var(--accent-terracotta); font-weight:600; margin-bottom:1rem;">${state.daysUntilReorder <= 0
            ? "Going by your usual rhythm, you're due for a resupply."
            : `Going by your usual rhythm, you're about ${state.daysUntilReorder} day${state.daysUntilReorder === 1 ? '' : 's'} from running low.`}</p>`
        : '';
    return panel(`
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(230px, 1fr)); gap:1rem;">
      ${state.recommendations
        .map((r) => `<div style="border:1px solid var(--border-subtle); border-radius:var(--radius-md); overflow:hidden; display:flex; flex-direction:column;">
...
function renderOrders() {
    const rows = state.orders.length === 0
        ? '<p style="color:var(--text-muted); margin:0;">No orders yet. Your history will appear here after your first roast.</p>'
        : state.orders
            .map((o) => `<div style="border:1px solid var(--border-subtle); border-radius:var(--radius-md); padding:1rem; margin-bottom:0.8rem;">
            ...
            .join('');
    const pager = state.ordersTotal > PAGE_SIZE
...
function renderPreferences() {
    ...
    <div style="display:flex; flex-direction:column; gap:0.5rem;">
      ${(prefs.channels || [])
        .map((ch) => `<label style="display:flex; align-items:center; gap:0.6rem; font-size:0.92rem;">
...
function addressForm(addr) {
    const field = (key, label, value, required = true) => `
...
function renderAddresses() {
    const rows = state.addresses.length === 0
...
async function loadOrders() {
    const res = await apiFetch(`/api/profile/orders?limit=${PAGE_SIZE}&offset=${state.offset}`);
    if (!res.success)
        return;
...
async function loadAll() {
    ...
    const [core, recs] = await Promise.all([
        ...
        apiFetch('/api/profile/recommendations?limit=4'),
    ]);
    if (!core.success) {
...
async function reorder(orderNumber, button) {
    ...
    const res = await apiFetch(`/api/profile/orders/${encodeURIComponent(orderNumber)}/reorder`, { method: 'POST' });
    ...
    for (const line of res.items || []) {
        const product = products.find((p) => p.id === line.product_id || (p.variants || []).some((v) => v.id === line.variant_id));
        const variant = product ? (product.variants || []).find((v) => v.id === line.variant_id) : null;
...
async function showOrderDetail(orderNumber) {
    const target = section?.querySelector(`[data-profile-detail="${CSS.escape(orderNumber)}"]`);
    if (!target)
        return;
    ...
    const res = await apiFetch(`/api/profile/orders/${encodeURIComponent(orderNumber)}`);
    if (!res.success) {
...
async function savePreferences() {
    const statusEl = section?.querySelector('#profile-pref-status');
    const grind = section?.querySelector('#profile-pref-grind')?.value || null;
    const weight = section?.querySelector('#profile-pref-weight')?.value || null;
    const brew = section?.querySelector('#profile-pref-brew')?.value || null;
    const channels = {};
    section?.querySelectorAll('[data-profile-channel]').forEach((input) => {
        channels[input.dataset.profileChannel] = input.checked;
    });
    if (statusEl)
...
async function saveAddress(id) {
    const container = section?.querySelector(`[data-profile-action="addr-save"][data-id="${CSS.escape(id)}"]`)?.parentElement?.parentElement;
    if (!container)
        return;
    const payload = {};
    container.querySelectorAll('[data-profile-addr-field]').forEach((input) => {
        payload[input.dataset.profileAddrField] = input.value.trim();
    });
    const res = await apiFetch(`/api/profile/addresses/${encodeURIComponent(id)}`, { method: 'PATCH', json: payload });
    if (!res.success) {
...
function onSectionClick(event) {
    ...
    switch (action) {
        ...
        case 'order-detail':
            ...
            break;
        case 'reorder':
            void reorder(orderNumber, target);
            break;
        case 'orders-prev':
