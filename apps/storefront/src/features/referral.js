...
function renderDashboard(host, d) {
  ...
  `;
    host.querySelector('#ref-copy')?.addEventListener('click', async () => {
        const note = host.querySelector('#ref-copied');
        try {
...
export function initReferral(app) {
    void app;
    injectStyles();
    captureInboundCode();
    ...
    void (async () => {
        if (!isSignedIn()) {
            const terms = await apiFetch('/api/referral/terms');
            renderSignedOut(section, terms.success ? terms.terms || null : null);
            return;
        }
        const res = await apiFetch('/api/referral/me');
