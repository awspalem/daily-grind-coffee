...
async function renderPlans(): Promise<void> {
  ...
  const res = await apiFetch<{ plans: any[] }>('/api/subscriptions/plans');
    ...
    <div class="plan-grid">
      ${plans.map((plan) => `
        <article class="plan-card${plan.badge ? ' is-featured' : ''}">
          ...
          <ul class="plan-perks">
            ${(plan.perks || []).map((perk: string) => `<li>${esc(perk)}</li>`).join('')}
          </ul>
  ...
  section.querySelectorAll<HTMLButtonElement>('[data-plan-slug]').forEach((btn) => {
...
async function startPlanCheckout(slug: string, btn: HTMLButtonElement): Promise<void> {
  ...
  const res = await apiFetch<{ checkout_url: string }>('/api/subscriptions/checkout', {
    method: 'POST',
...
async function pickStartingVariant(): Promise<string | null> {
  if (cachedVariantId) return cachedVariantId;

  const list = await apiFetch<{ products: any[] }>('/api/products');
  const first = (list.success ? list.products || [] : [])[0];
  if (!first?.slug) return null;

  const detail = await apiFetch<{ product: any }>(`/api/products/${encodeURIComponent(first.slug)}`);
...
async function renderManager(): Promise<void> {
  ...
  const [subsRes, perksRes] = await Promise.all([
    apiFetch<{ subscriptions: any[] }>('/api/subscriptions/mine'),
    apiFetch<{ perks: any[] }>('/api/subscriptions/perks'),
  ]);
...
function renderPerks(perks: any[]): string {
...
function renderSubscriptionCard(sub: any): string {
          ...
          <label>Frequency
            <select data-field="frequency">
              ${Object.entries(FREQUENCY_LABELS).map(([value, label]) =>
                `<option value="${esc(value)}"${value === sub.frequency ? ' selected' : ''}>${esc(label)}</option>`).join('')}
            </select>
          </label>
...
function wireManager(section: HTMLElement): void {
  section.querySelectorAll<HTMLElement>('.sub-card').forEach((card) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        if (action === 'save') {
          ...
          card.querySelectorAll<HTMLInputElement | HTMLSelectElement>('[data-field]').forEach((field) => {
            const key = field.dataset.field as string;
        ...
        if (action === 'cancel') { await runCancelFlow(btn, subId); return; }
        if (action === 'fix-payment') { await runPaymentFix(btn, subId); return; }
...
async function act(btn: HTMLButtonElement, path: string, method: string, json?: unknown): Promise<boolean> {
  const original = btn.textContent;
  btn.disabled = true;
  btn.textContent = 'Working…';

  const res = await apiFetch<{ message?: string }>(path, { method, json: json ?? {} });
 ...
 */
async function runCancelFlow(btn: HTMLButtonElement, subId: string): Promise<void> {
  const reason = prompt(
    'Before you go — what is prompting this?\n(e.g. price, too much coffee, moving house, taking a break)'
  );
  if (reason === null) return;

  const offerRes = await apiFetch<{ offer: any }>(`/api/subscriptions/${subId}/cancel-offer`, {
    method: 'POST',
    json: { reason },
  });
  ...
  if (!confirm('Cancel this subscription for good?')) return;
  await act(btn, `/api/subscriptions/${subId}/cancel`, 'POST', { reason });
}

async function runPaymentFix(btn: HTMLButtonElement, subId: string): Promise<void> {
  const original = btn.textContent;
  btn.disabled = true;
  btn.textContent = 'Opening…';

  const res = await apiFetch<{ checkout_url: string; session_id: string }>(
    `/api/subscriptions/${subId}/payment-method/session`, { method: 'POST', json: {} }
  );
...
async function completePendingPaymentFix(): Promise<void> {
  const params = new URLSearchParams(window.location.search);
  if (!params.get('subscription_payment_fixed')) return;
  ...
  const res = await apiFetch<{ message: string }>(
    `/api/subscriptions/${subId}/payment-method/confirm`, { method: 'POST', json: { session_id: sessionId } }
  );
  if (res.success) alert(res.message || 'Payment method updated.');
}

export function initSubscriptions(app: any): void {
  void app;
  injectStyles();
