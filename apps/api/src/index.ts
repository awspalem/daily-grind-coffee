...
export default {
  ...
  async scheduled(controller: ScheduledController, env: Env, ctx: ExecutionContext): Promise<void> {
    ...
    try {
      ...
      `).bind(staleThreshold).all<{ id: string; order_number: string; customer_email: string }>();
      ...
      if (staleOrders && staleOrders.length > 0) {
        ...
        for (const order of staleOrders) {
          const { results: items } = await env.DB.prepare(
            'SELECT variant_id, quantity, product_name, weight_grams, grind_type FROM order_items WHERE order_id = ?'
          ).bind(order.id).all<{ variant_id: string; quantity: number; product_name: string; weight_grams: number; grind_type: string }>();
    ...
    try {
      ...
      `).all<{ id: string; order_number: string; customer_email: string }>();
      ...
      if (deliveredOrders && deliveredOrders.length > 0) {
        ...
        for (const order of deliveredOrders) {
          ...
          await env.DB.prepare(
            'UPDATE orders SET review_request_sent_at = CURRENT_TIMESTAMP WHERE id = ?'
          ).bind(order.id).run();
        }
