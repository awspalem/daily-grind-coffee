...
app.use('*', cors({
    ...
    allowHeaders: ['Content-Type', 'Authorization', 'X-Session-Token', 'X-Turnstile-Token', 'X-Customer-Session', 'stripe-signature', 'Cf-Access-Jwt-Assertion'],
      ...
      `).all();
            if (dueSubs && dueSubs.length > 0) {
                ...
                for (const sub of dueSubs) {
                    const markPastDue = async (reason) => {
                        pastDue++;
                        console.warn(`[CRON SUBSCRIPTION] ${sub.id} (${sub.product_name}) marked PAST_DUE: ${reason}`);
                        await env.DB.prepare("UPDATE subscriptions SET status = 'PAST_DUE', updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(sub.id).run();
                    };
                    if (!sub.stripe_customer_id || !sub.stripe_payment_method_id) {
                        await markPastDue('no saved payment method on file');
                        continue;
                    }
                    const variant = await env.DB.prepare('SELECT price_cents FROM product_variants WHERE id = ? AND is_active = 1').bind(sub.variant_id).first();
                    if (!variant) {
            ...
            `).bind(orderId, orderNumber, sub.customer_email, totalCents, shippingCents, taxCents, orderTotalCents, sub.shipping_address_json || '{}', charge.paymentIntentId, `Subscription renewal for ${sub.id} (${sub.frequency.replace('_', ' ')})`),
                        env.DB.prepare(`
            ...
            `).bind('oi_' + crypto.randomUUID().replace(/-/g, '').slice(0, 16), orderId, sub.variant_id, sub.product_name, sub.variant_id, sub.grind_type, unitPriceCents, sub.quantity, totalCents, sub.frequency),
                    ]);
                    ...
                    await env.DB.prepare('UPDATE subscriptions SET next_renewal_date = ?, order_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').bind(nextRenewalDate, orderId, sub.id).run();
                    renewed++;
                    try {
                        const emailData = generateOrderConfirmationEmail({
                            orderNumber,
