 ...
 */
async function insertMessage(db, opts) {
    ...
    await db
        ...
        .bind('am_' + crypto.randomUUID().replace(/-/g, '').slice(0, 20), opts.sessionToken, opts.customerId || null, opts.role, opts.content, opts.actorType, opts.actorId || null)
        .run();
}
/**
 ...
 */
export async function saveAgentTurn(db, opts) {
    ...
    try {
        ...
        await db
            .prepare(`DELETE FROM agent_messages
          WHERE session_token = ?
            AND actor_type = ?
            AND id NOT IN (
 ...
 */
export async function loadAgentHistory(db, opts) {
    if (!opts.sessionToken)
        return [];
    const actorType = opts.actorType || 'customer';
    try {
