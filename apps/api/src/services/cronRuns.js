...
export async function recordCronRun(env, run) {
    if (!env.DB)
        throw new Error('recordCronRun called without D1 binding');
    const result = await env.DB.prepare(`
    INSERT INTO cron_runs (
      job_name, trigger_type, status, started_at, finished_at, duration_ms, error_message, metadata_json
    ) VALUES (?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), ?, ?, ?, ?)
  `).bind(run.job_name, run.trigger_type, run.status, run.started_at || null, run.finished_at || null, run.duration_ms ?? null, run.error_message || null, run.metadata ? JSON.stringify(run.metadata) : null).run();
    return Number(result?.meta?.last_row_id ?? 0);
}
/**
 * Returns the most recent run per job — used by /api/health.
 */
export async function getLastCronRuns(env) {
    if (!env.DB)
        return [];
