...
/**
 * Snapshot of `loyalty_config`, shaped to drop in for `LOYALTY_RATES` so callers can use either
 * interchangeably. Reads from the DB; falls back to the in-code constants when the row is absent
 * (fresh dev environments, before the seed runs).
 */
export interface LoyaltyRuntimeConfig {
  RUPEES_PER_POINT_EARNED: number;
  POINT_VALUE_CENTS: number;
  SIGNUP_BONUS_POINTS: number;
  REVIEW_BONUS_POINTS: number;
  SUBSCRIPTION_STREAK_POINTS: number;
  SUBSCRIPTION_STREAK_EVERY: number;
  MIN_REDEEM_POINTS: number;
  MAX_REDEEM_PERCENT: number;
  EXPIRY_MONTHS: number;
  REDEMPTION_HOLD_MINUTES: number;
  TIER_THRESHOLDS_CENTS: Record<LoyaltyTier, number>;
  TIER_MULTIPLIERS: Record<LoyaltyTier, number>;
}
...
interface LoyaltyConfigRow {
  ...
  subscription_streak_every: number;
  redemption_hold_minutes: number;
  tier_threshold_silver_cents: number;
  tier_threshold_gold_cents: number;
  tier_multiplier_silver: number;
  tier_multiplier_gold: number;
}

export async function loadLoyaltyConfig(db: Env['DB']): Promise<LoyaltyRuntimeConfig> {
  const row = await db
 ...
 */
export async function tierHistory(
  db: Env['DB'],
  customerId: string,
  limit = 10
): Promise<Array<{ id: string; from_tier: LoyaltyTier | null; to_tier: LoyaltyTier; trailing_spend_cents: number; created_at: string }>> {
  const { results } = await db
    .prepare(`
...
export async function getSummary(db: Env['DB'], customerId: string): Promise<LoyaltySummary | null> {
  ...
  return {
    ...
    point_value_cents: LOYALTY_RATES.POINT_VALUE_CENTS,
    recent_entries: (recent?.results || []) as LoyaltyLedgerEntry[],
  };
}
