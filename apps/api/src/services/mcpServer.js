            ...
            try {
                if (toolName === 'search_products') {
                    ...
                    if (args.query) {
                        ...
                        res = products.filter((p) => p.name.toLowerCase().includes(q) ||
                ...
                if (toolName === 'get_order') {
                    const order = await this.db.prepare('SELECT order_number, status, total_cents, tracking_number, carrier, created_at FROM orders WHERE order_number = ?').bind(args.order_number).first();
                    return {
                        jsonrpc: '2.0',
                        id: req.id,
                        result: {
