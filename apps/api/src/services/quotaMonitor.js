export class FreeTierQuotaMonitor {
    ...
    async getUsageReport() {
        ...
        const analyticsCountRow = await this.db.prepare('SELECT COUNT(*) as count FROM analytics_events WHERE created_at >= ?').bind(dateIso).first();
        const ordersCountRow = await this.db.prepare('SELECT COUNT(*) as count FROM orders WHERE created_at >= ?').bind(dateIso).first();
        const movementsCountRow = await this.db.prepare('SELECT COUNT(*) as count FROM inventory_movements WHERE created_at >= ?').bind(dateIso).first();
