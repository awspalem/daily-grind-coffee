...
function normalizeAddress(line1: string | null | undefined, postal: string | null | undefined): string {
 ...
 */
export async function getOrCreateCode(db: Env['DB'], customerId: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const res = await db
      ...
      .run();
    if ((res as any)?.meta?.changes) return code;

    const row = await db
      .prepare('SELECT code FROM referral_codes WHERE customer_id = ?')
      .bind(customerId)
      .first<{ code: string }>();
    if (row) return row.code; // lost the race to our own concurrent request
  }
  throw new Error('Could not allocate a referral code');
}

export function buildShareTargets(code: string, storefrontUrl: string): ReferralShareTargets {
  const url = `${storefrontUrl.replace(/\/$/, '')}/?ref=${encodeURIComponent(code)}`;
  const message =
    `I'm drinking small-batch coffee roasted in Bangalore by The Daily Roast. ` +
...
export async function recordVisit(db: Env['DB'], code: string, visitorHash: string): Promise<boolean> {
...
export interface ReferralCheckResult extends ReferralValidation {
  referrerCustomerId?: string;
  blockedReason?: ReferralBlockReason;
}
 ...
 */
export async function checkReferral(db: Env['DB'], input: ReferralCheckInput): Promise<ReferralCheckResult> {
  const code = String(input.code || '').trim().toUpperCase();
  const refereeEmail = normalizeEmail(input.refereeEmail);
  const fail = (error: string, blockedReason?: ReferralBlockReason): ReferralCheckResult => ({
    valid: false,
    error,
    discount_cents: 0,
    blockedReason,
  ...
  if (!code) return fail('Referral code required');
  if (!refereeEmail) return fail('Email required to apply a referral code');
  ...
  const owner = await db
    ...
    `)
    .bind(code)
    .first<{ customer_id: string; email: string; phone: string | null; full_name: string | null }>();
  if (!owner) return fail('That referral code is not valid');
  ...
  const priorReferral = await db
    ...
    .bind(refereeEmail)
    .first<{ id: string }>();
  if (priorReferral) return fail('This email has already used a referral code', 'ALREADY_REFERRED');
  ...
  const priorOrder = await db
    ...
    .bind(refereeEmail)
    .first<{ id: string }>();
  if (priorOrder) return fail('Referral codes are for first orders only', 'EXISTING_CUSTOMER');
  ...
  if (refereeAddress !== '|' && refereeAddress.length > 4) {
    const { results: referrerOrders } = await db
      ...
      .all<{ shipping_address_json: string }>();
 ...
 */
export function referralAttachStatement(
  db: Env['DB'],
  input: {
    referrerCustomerId: string;
    code: string;
  ...
  return db
    ...
    .bind(
      ...
      input.code,
      input.refereeCustomerId ?? null,
      normalizeEmail(input.refereeEmail),
      normalizePhone(input.refereePhone) || null,
      input.orderId,
      input.discountCents
    );
}
 ...
 */
export async function qualifyReferral(db: Env['DB'], orderId: string): Promise<number> {
...
export async function reverseReferral(db: Env['DB'], orderId: string): Promise<number> {
  const referral = await db
...
export async function orderHasReferral(db: Env['DB'], orderId: string): Promise<boolean> {
  const row = await db.prepare('SELECT id FROM referrals WHERE order_id = ?').bind(orderId).first<{ id: string }>();
  return Boolean(row);
}
...
export async function getDashboard(
  ...
  const { results: recent } = await db
    ...
    `)
    .bind(customerId)
    .all<{ referee_email_norm: string; status: any; referrer_points_awarded: number; created_at: string }>();
