...
export function addMonths(from: string | Date, months: number): string {
...
export function serialisePlan(row: PlanRow) {
  return {
    id: row.id,
    slug: row.slug,
    name: row.name,
    ...
    perks: safeJson<string[]>(row.perks_json, []),
    entitlements: safeJson<PlanEntitlementSpec[]>(row.entitlements_json, []),
    badge: row.badge,
...
export async function listPlans(db: Env['DB'], includeInactive = false): Promise<PlanRow[]> {
  const sql = `
  ...
  return (results || []) as PlanRow[];
}

export async function getPlan(db: Env['DB'], idOrSlug: string): Promise<PlanRow | null> {
  return await db
...
export async function listOwnedSubscriptions(
  db: Env['DB'],
  session: { customerId: string; email: string }
...
export async function serialiseSubscription(db: Env['DB'], sub: SubscriptionRow) {
  ...
  return {
    ...
    plan_perks: plan ? safeJson<string[]>(plan.perks_json, []) : [],
    ...
    shipping_address: safeJson<Record<string, unknown> | null>(sub.shipping_address_json, null),
...
export async function grantPlanEntitlements(
  ...
  params: {
    ...
    subscriptionId: string;
    plan: PlanRow;
    sourceType: 'SUBSCRIPTION' | 'PLAN_RENEWAL';
    startsAt: string;
    expiresAt: string;
  }
): Promise<{ granted: string[]; skipped: string[] }> {
  const specs = safeJson<PlanEntitlementSpec[]>(params.plan.entitlements_json, []);
  const granted: string[] = [];
  const skipped: string[] = [];

  for (const spec of specs) {
    if (!spec?.code || typeof spec.units !== 'number' || spec.units === 0) continue;
...
export async function getPlanPerkBalances(db: Env['DB'], customerId: string) {
...
export async function activatePlanSubscription(
  ...
  await db
    ...
    .bind(
      ...
      isAnnual ? startsAt : addDays(startsAt, days),
      isAnnual ? plan.shipments_included : null,
      sub.id
    )
    .run();
}
 ...
 */
export function projectUpcomingShipments(sub: SubscriptionRow, count = 3) {
  ...
  for (let i = 0; i < count; i++) {
    if (remaining !== null && remaining !== undefined && i >= remaining) break;
    if (sub.term_ends_at && new Date(cursor) > new Date(sub.term_ends_at)) break;
...
export async function pauseSubscription(db: Env['DB'], sub: SubscriptionRow, actor: 'CUSTOMER' | 'ADMIN' = 'CUSTOMER') {
  await db
    .prepare(`
      UPDATE subscriptions
 ...
 */
export async function resumeSubscription(db: Env['DB'], sub: SubscriptionRow, actor: 'CUSTOMER' | 'ADMIN' = 'CUSTOMER') {
  const days = FREQUENCY_DAYS[sub.frequency] ?? 14;
 ...
 */
export async function skipNextDelivery(db: Env['DB'], sub: SubscriptionRow, actor: 'CUSTOMER' | 'ADMIN' = 'CUSTOMER') {
  const days = FREQUENCY_DAYS[sub.frequency] ?? 14;
...
export async function updateSubscription(
  ...
  if (input.grind_type !== undefined) {
    if (!GRIND_TYPES.includes(input.grind_type)) return { ok: false, error: 'Unknown grind type' };
  ...
  if (input.frequency !== undefined) {
    if (!FREQUENCY_DAYS[input.frequency]) return { ok: false, error: 'Unknown delivery frequency' };
    sets.push('frequency = ?');
  ...
  if (input.quantity !== undefined) {
    ...
    if (!Number.isFinite(qty) || qty < 1 || qty > 10) return { ok: false, error: 'Quantity must be between 1 and 10' };
    sets.push('quantity = ?');
    binds.push(qty);
  ...
  if (input.next_renewal_date !== undefined) {
    ...
    if (Number.isNaN(when.getTime())) return { ok: false, error: 'Invalid delivery date' };
    // Never let a customer set a date in the past — the cron would bill it on its next run.
    if (when.getTime() < Date.now()) return { ok: false, error: 'Pick a date in the future' };
...
export async function swapCoffee(
  db: Env['DB'],
  sub: SubscriptionRow,
  variantId: string,
  actor: 'CUSTOMER' | 'ADMIN' = 'CUSTOMER'
): Promise<{ ok: true; product_name: string } | { ok: false; error: string }> {
  const variant = await db
    ...
    `)
    .bind(variantId)
    .first<{ id: string; price_cents: number; weight_grams: number; product_name: string }>();
  if (!variant) return { ok: false, error: 'That coffee is not available' };
 ...
 */
export function buildSaveOffer(sub: SubscriptionRow, reason: string) {
  if (sub.save_offer_used_at) return null;

  const normalised = (reason || '').toUpperCase();
  if (normalised.includes('TOO_MUCH') || normalised.includes('TOO_OFTEN')) {
    return {
      kind: 'SLOWER_CADENCE' as const,
...
export async function acceptSaveOffer(
  ...
  if (sub.save_offer_used_at) return { ok: false, error: 'This offer has already been used' };
...
export async function restorePaymentMethod(
  db: Env['DB'],
  sub: SubscriptionRow,
  billing: { customerId: string | null; paymentMethodId: string | null }
): Promise<{ ok: true } | { ok: false; error: string }> {
 ...
 */
export async function sendUpcomingRenewalNotices(env: Env, daysAhead = 3): Promise<number> {
  ...
  `).bind(cutoff).all<SubscriptionRow>();

  const due = (results || []) as SubscriptionRow[];
  ...
  for (const sub of due) {
    ...
    const res = await email.send(
      sub.customer_email,
      `Your next Daily Roast delivery ships ${new Date(next.scheduled_for).toDateString()}`,
      renderRenewalNoticeHtml(sub, next, env.STOREFRONT_URL || 'https://dailyroast.in')
    );
    if (!res.success) {
    ...
    await env.DB.prepare(
      'UPDATE subscriptions SET renewal_notice_sent_for = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
    ).bind(sub.next_renewal_date, sub.id).run();
...
export function renderRenewalNoticeHtml(
  sub: SubscriptionRow,
  shipment: { scheduled_for: string; estimated_total_cents: number; will_charge: boolean },
 ...
 */
export async function processPrepaidShipments(env: Env): Promise<{ shipped: number; exhausted: number }> {
  const { results } = await env.DB.prepare(`
    SELECT * FROM subscriptions
  ...
  for (const sub of due) {
    ...
    await env.DB.batch([
      ...
      `).bind(
        orderId, orderNumber, sub.customer_email, lineTotal, lineTotal,
        sub.shipping_address_json || '{}',
        `Prepaid plan shipment for ${sub.id}`
      ),
      env.DB.prepare(`
      ...
      `).bind(
        newId('oi'), orderId, sub.variant_id, sub.product_name, sub.variant_id, sub.grind_type,
        Math.round(sub.unit_price_cents * (1 - sub.discount_percent / 100)), sub.quantity,
        lineTotal, sub.frequency
      ),
      env.DB.prepare(`
        UPDATE subscriptions
        ...
        WHERE id = ?
      `).bind(
        addDays(sub.next_renewal_date, FREQUENCY_DAYS[sub.frequency] ?? 14), orderId, sub.id
      ),
    ]);
