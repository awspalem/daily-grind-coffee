...
function isConfigured(email?: string, password?: string): boolean {
...
export class ShiprocketService {
  constructor(
    ...
    private usdToInrRate: number = 83
  ) {}
  ...
  async createOrder(input: ShiprocketOrderInput): Promise<ShiprocketOrderResult> {
    if (!this.configured) {
      if (!this.allowMock) {
    ...
    if (!input.customerPhone) {
      throw new Error(
        'Order has no customer phone number on file — Shiprocket requires one for courier contact. ' +
    ...
    const toInr = (cents: number): number => {
  ...
  async trackShipment(shipmentId: string): Promise<ShiprocketTrackingResult> {
    ...
    const data = await this.request<any>(`/courier/track/shipment/${shipmentId}`);
