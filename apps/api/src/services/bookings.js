...
export function formatInRoasteryTime(value, timeZone = ROASTERY_TIMEZONE) {
...
export function formatDateInRoasteryTime(value, timeZone = ROASTERY_TIMEZONE) {
    const d = toDate(value);
    if (!d)
        return '';
...
// Reads
// ---------------------------------------------------------------------------
export async function getExperience(db, slugOrId) {
    return db
        .prepare('SELECT * FROM experiences WHERE (slug = ? OR id = ?) LIMIT 1')
        .bind(slugOrId, slugOrId)
        .first();
}
export async function listExperiences(db, includeInactive = false) {
 ...
 */
export async function listAvailableSlots(db, experienceId, opts = {}) {
...
export async function getBookingDetail(db, bookingId) {
    return db.prepare(`${BOOKING_DETAIL_SQL} WHERE b.id = ?`).bind(bookingId).first();
}
export async function listCustomerBookings(db, customerId) {
...
export function seatPriceCents(experience, slot) {
...
export function withinCancellationWindow(startsAt, cutoffHours) {
...
export async function createBooking(db, input) {
    ...
    if (slot.status !== 'OPEN')
        return { success: false, error: 'That slot is no longer open for booking.' };
  ...
  `).bind(bookingId, reference, experience.id, customerId, input.customerEmail, input.customerName ?? null, input.contactPhone ?? null, requested, requested, amountCents, depositCents, experience.currency, experience.entitlement_code, input.dietaryNotes ?? null, input.accessibilityNotes ?? null, holdExpiresAt, icsToken, key, slot.id, requested);
...
async function createWaitlistEntry(db, input) {
  ...
  `).bind(bookingId, newBookingReference(), experience.id, slot.id, input.customerId, input.customerEmail, input.customerName ?? null, input.contactPhone ?? null, input.seats, input.seats, input.amountCents, input.depositCents, experience.currency, experience.entitlement_code, input.dietaryNotes ?? null, input.accessibilityNotes ?? null, crypto.randomUUID().replace(/-/g, ''), input.idempotencyKey || `auto_${bookingId}`).run();
    const booking = await getBookingDetail(db, bookingId);
 ...
 */
export async function confirmBooking(env, booking, opts = {}) {
    const db = env.DB;
    if (booking.status === 'CONFIRMED') {
        return { success: true, booking, fundingSource: booking.funding_source || undefined };
    }
...
async function finaliseConfirmation(env, booking, fundingSource) {
 ...
 */
export async function settleBookingPayment(env, booking) {
    if (booking.status === 'CONFIRMED')
        return { success: true, booking, fundingSource: 'PAID' };
    if (!booking.stripe_session_id)
        return { success: false, error: 'No payment session for this booking.' };
 ...
 */
async function fetchSessionPaymentState(env, sessionId) {
    const key = env.STRIPE_SECRET_KEY;
    const isMock = !key || key.startsWith('sk_test_mock') || key === 'placeholder' || sessionId.startsWith('cs_mock_');
    if (isMock)
        return { paid: true, paymentIntentId: 'pi_mock_' + sessionId.slice(-16) };
    try {
 ...
 */
export async function cancelBooking(env, booking, opts = {}) {
    const db = env.DB;
  ...
  `).bind(opts.reason ?? (opts.byStaff ? 'Cancelled by staff' : 'Cancelled by customer'), booking.id).run();
    if (heldSeat)
        await releaseSeats(db, booking.slot_id, booking.seats);
    ...
    const updated = await getBookingDetail(db, booking.id);
    if (updated)
        await sendCancellationEmail(env, updated, { entitlementRestored, refundStatus });
...
async function restoreEntitlementForBooking(db, booking) {
...
async function refundBookingPayment(env, booking) {
    const key = env.STRIPE_SECRET_KEY;
    ...
    try {
        ...
        if (!res.ok)
            console.error('[bookings] refund failed', await res.text());
...
async function releaseSeats(db, slotId, seats) {
    await db.prepare(`
    UPDATE experience_slots
 ...
 */
export async function rescheduleBooking(env, booking, newSlot) {
    const db = env.DB;
  ...
  `).bind(seats, oldSlotId, booking.id, newSlot.id);
    ...
    if (updated && updated.status === 'CONFIRMED')
        await sendConfirmationEmail(env, updated, { rescheduled: true });
    return { success: true, booking: updated ?? undefined };
}
/** Staff-side attendance marking (5.5). */
export async function markAttendance(db, bookingId, outcome, staffNotes) {
 ...
 */
export async function promoteFromWaitlist(env, slotId) {
    const db = env.DB;
  ...
  `).bind(next.seats, slotId, next.seats);
    ...
    if (changesOf(offerResult) === 0)
        return null;
    const detail = await getBookingDetail(db, next.id);
    if (detail)
        await sendWaitlistOfferEmail(env, detail);
    return next.id;
}
// ---------------------------------------------------------------------------
// 5.4 — Calendar invite
...
function detailsTable(booking, apiBase) {
    const isVideo = booking.mode === 'VIDEO';
    ...
    if (booking.party_size > 1)
        rows.push(['Party', `${booking.party_size} people`]);
    if (booking.staff_name)
        rows.push(['Your host', escHtml(booking.staff_name)]);
    if (booking.funding_source === 'ENTITLEMENT') {
    ...
    if (booking.dietary_notes)
        rows.push(['Dietary notes', escHtml(booking.dietary_notes)]);
    if (booking.accessibility_notes)
        rows.push(['Accessibility', escHtml(booking.accessibility_notes)]);
...
export function generateBookingConfirmationEmail(booking, apiBase, opts = {}) {
...
export function generateBookingReminderEmail(booking, apiBase) {
    const isVideo = booking.mode === 'VIDEO';
    return {
        to: booking.customer_email,
...
export function generateWaitlistOfferEmail(booking, storefrontUrl) {
...
export function generateBookingCancellationEmail(booking, opts) {
...
export async function sendConfirmationEmail(env, booking, opts = {}) {
...
async function sendWaitlistOfferEmail(env, booking) {
...
async function sendCancellationEmail(env, booking, opts) {
 ...
 */
export async function sendDueReminders(env, horizonHours = 24) {
 ...
 */
export async function expireStaleHolds(env) {
    const { results } = await env.DB.prepare(`
  ...
  `).bind(nowIso()).all();
    ...
    `).bind(row.id).run();
        if (changesOf(res) === 0)
            continue; // someone confirmed it in the meantime
