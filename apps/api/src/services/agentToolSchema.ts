...
/**
 * The public tool list — a hand-mirror of `AGENT_TOOLS` in apps/api/src/routes/agent.ts.
 *
 * Each entry carries:
 *   - `name`        matching the tool name Maya invokes
 *   - `description` matching the runtime description (one-liner Maya reads)
 *   - `inputSchema`  JSON Schema 2020-12 for the tool's arguments
 *   - `confirmationRequired` true for tools that produce a proposed action (add to cart,
 *     cancel subscription) so callers know they cannot fire-and-forget
 */
export interface PublicAgentTool {
  name: string;
  description: string;
  /** JSON Schema 2020-12 document describing the tool's input object. */
  inputSchema: Record<string, unknown>;
  /** True when the tool returns a `proposed_action` that the user must confirm before
   *  it mutates anything. */
  confirmationRequired: boolean;
}

export const PUBLIC_AGENT_TOOLS: PublicAgentTool[] = [
  {
    name: 'semantic_coffee_search',
    description:
      'Search coffee catalog and brewing guides using vector embedding similarity (e.g. "something sweet with jasmine and low acidity").',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'search_coffee',
    description:
      'Search catalog by roast level, tasting notes, origin, or flavor keywords.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    description:
      'Retrieve detailed brew ratios, water temperatures, and step-by-step instructions for a brew method.',
    inputSchema: {
      type: 'object',
      properties: {
        method: {
  ...
  },
  {
    ...
    description:
      'Propose adding a specific coffee variant and grind to the cart. Returns a confirmation token; the customer must confirm in the storefront before the cart is mutated.',
    inputSchema: {
      type: 'object',
      properties: {
        variant_id: {
          type: 'string',
          description:
            'Variant ID (e.g. var_att_250, var_ara_250, var_eth_250, var_dawn_250, var_mid_250)',
        },
        ...
        notes: {
          ...
          maxLength: 200,
          description:
            'Optional free-text note for the cart line (e.g. "gift wrap", "no filter papers"). ≤200 chars.',
        },
      },
  ...
  },
  {
    ...
    description:
      "Look up the customer's active subscriptions: which coffee, frequency, next renewal date, and current status (ACTIVE / PAUSED / PAST_DUE / CANCELLED). Use this when the customer asks about their subscription, when the next delivery is, or whether a subscription is still running.",
    inputSchema: {
      type: 'object',
      properties: {
        subscription_id: {
          type: 'string',
          description:
            "Optional: a specific subscription id. Omit to list all of the caller's subscriptions.",
        },
      },
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    description:
      "Look up the customer's current loyalty points balance, lifetime total, and tier. Use this when the customer asks how many points they have or what tier they are on.",
    inputSchema: {
      type: 'object',
      properties: {},
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      'Get personalised coffee recommendations derived from the customer\'s purchase and review history. Use this when the customer asks for a recommendation, a "what should I try next" or "what do you think I\'d like" question, especially after they have ordered before.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: {
  ...
  },
  {
    ...
    description:
      "Propose cancelling one of the customer's active subscriptions. Returns a confirmation token; the customer must confirm in the storefront before the subscription is cancelled. NEVER cancel without a real user request — every cancellation stops a future recurring charge and a future shipment.",
    inputSchema: {
      type: 'object',
      properties: {
 ...
 */
export function publicToolsAsGroqDefinitions(): GroqToolDefinition[] {
