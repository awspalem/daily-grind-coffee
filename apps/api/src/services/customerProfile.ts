 ...
 */
const COUNTED_STATUSES = ['PAID', 'ROASTING', 'PACKED', 'SHIPPED', 'DELIVERED'] as const;
...
export function orderOwnerBindings(customerId: string, email: string): [string, string] {
  return [customerId, email.trim().toLowerCase()];
}

/** Ranks a tally into a share-normalised distribution, biggest first. */
function toDistribution(tally: Map<string, number>, limit = 8): TasteAffinity[] {
  let total = 0;
  for (const units of tally.values()) total += units;
  if (total <= 0) return [];
...
export function classifySegment(input: {
  ...
  if (daysSinceLastOrder > AT_RISK_MAX_DAYS) return 'LAPSED';
  if (daysSinceLastOrder > ACTIVE_MAX_DAYS) return 'AT_RISK';

  if (totalOrders >= VIP_MIN_ORDERS && lifetimeValueCents >= VIP_MIN_LTV_CENTS) return 'VIP';
...
export function meanReorderCadenceDays(orderedAscIso: string[]): number | null {
  if (orderedAscIso.length < 2) return null;
...
export async function computeTasteProfile(
  ...
  const roastTally = new Map<string, number>();
  const originTally = new Map<string, number>();
  const processTally = new Map<string, number>();
  const productTally = new Map<string, number>();
  const grindTally = new Map<string, number>();
  const weightTally = new Map<string, number>();
  ...
  for (const it of items) {
    ...
    if (it.roast_level) roastTally.set(it.roast_level, (roastTally.get(it.roast_level) || 0) + units);
    if (it.origin_country) originTally.set(it.origin_country, (originTally.get(it.origin_country) || 0) + units);
    if (it.process_method) processTally.set(it.process_method, (processTally.get(it.process_method) || 0) + units);
    if (it.product_id) productTally.set(it.product_id, (productTally.get(it.product_id) || 0) + units);
    if (it.grind_type) grindTally.set(it.grind_type, (grindTally.get(it.grind_type) || 0) + units);
    if (it.weight_grams) weightTally.set(String(it.weight_grams), (weightTally.get(String(it.weight_grams)) || 0) + units);
  }
  ...
  let topRatedProductId: string | null = null;
  let bestRating = -1;
...
export async function saveTasteProfile(db: Env['DB'], profile: CustomerTasteProfile): Promise<void> {
  await db
    .prepare(
    ...
    .bind(
      ...
      profile.lifetime_value_cents,
      profile.aov_cents,
      profile.first_order_at,
      profile.last_order_at,
      profile.days_since_last_order,
      profile.reorder_cadence_days,
      profile.favourite_grind,
      profile.typical_weight_grams,
      profile.top_roast_level,
      profile.top_origin_country,
      profile.top_product_id,
      JSON.stringify(profile.roast_distribution),
      JSON.stringify(profile.origin_distribution),
      JSON.stringify(profile.process_distribution),
      JSON.stringify(profile.product_affinity),
      profile.review_count,
      profile.avg_review_rating,
...
function parseAffinities(raw: unknown): TasteAffinity[] {
...
function rowToProfile(row: any): CustomerTasteProfile {
  return {
    ...
    segment: (row.segment || 'NEW') as CustomerSegment,
...
function humanGrind(value: string | null): string | null {
 ...
 * the prompt nothing.
 */
export function summariseProfileForAgent(
  profile: CustomerTasteProfile,
  prefs?: { default_grind?: string | null; default_weight_grams?: number | null; brew_method?: string | null } | null
): string | null {
  const facts: string[] = [];
  ...
  if (profile.top_roast_level) facts.push(`prefers ${profile.top_roast_level.replace(/_/g, ' ').toLowerCase()} roasts`);
  if (profile.top_origin_country) facts.push(`buys most often from ${profile.top_origin_country}`);

  const grind = prefs?.default_grind || profile.favourite_grind;
  if (grind) facts.push(`usual grind ${humanGrind(grind)}`);

  const weight = prefs?.default_weight_grams || profile.typical_weight_grams;
  if (weight) facts.push(`usual bag ${weight}g`);

  if (prefs?.brew_method) facts.push(`brews with ${prefs.brew_method}`);
  if (profile.reorder_cadence_days) facts.push(`reorders roughly every ${Math.round(profile.reorder_cadence_days)} days`);
  if (profile.days_since_last_order !== null) facts.push(`last ordered ${profile.days_since_last_order} days ago`);
  if (profile.avg_review_rating !== null) facts.push(`rates our coffees ${profile.avg_review_rating}/5 on average`);
