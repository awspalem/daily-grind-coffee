import { activatePlanSubscription, getPlan, grantPlanEntitlements, revokePlanEntitlements, recordSubscriptionEvent, addMonths, } from '../services/subscriptionPlans';
/**
...
async function planSubscriptionsForOrder(env, orderId) {
    const { results } = await env.DB.prepare('SELECT * FROM subscriptions WHERE order_id = ? AND plan_id IS NOT NULL').bind(orderId).all();
    return (results || []);
}
export const plansHooks = {
    async onOrderPaid(env, ctx) {
        ...
        for (const sub of subs) {
            ...
            const customerId = sub.customer_id
                || (await env.DB.prepare('SELECT id FROM customers WHERE LOWER(email) = LOWER(?)')
                    .bind(sub.customer_email).first())?.id;
            if (!customerId) {
