...
test('agent-discovery: GET /api/agent/tools lists propose_add_to_cart with its schema URL', async () => {
    ...
    const propose = body.tools.find((t) => t.name === 'propose_add_to_cart');
    ...
    assert.equal(propose.schema_url, 'https://api.dailyroast.in/api/agent/tools/propose_add_to_cart/schema.json');
...
test('agent-discovery: GET /api/agent/openapi.json is a valid OpenAPI 3.1 document', async () => {
    ...
    assert.ok(requestDef.properties.messages || requestDef.properties.message, 'chat request must accept either messages or message');
    ...
    assert.ok(stream.responses['200'].content['text/event-stream'], 'streaming path must declare text/event-stream content type');
