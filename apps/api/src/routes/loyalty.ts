...
loyaltyApp.post('/redeem/preview', async (c) => {
  ...
  const body = (await c.req.json().catch(() => ({}))) as { subtotal_cents?: number };
...
loyaltyApp.post('/claim-review', async (c) => {
  ...
  const body = (await c.req.json().catch(() => ({}))) as { review_id?: string };
  if (!body.review_id) return c.json({ success: false, error: 'review_id required' }, 400);

  const review = await c.env.DB.prepare('SELECT id, order_number FROM reviews WHERE id = ?')
    .bind(body.review_id)
    .first<{ id: string; order_number: string | null }>();
  ...
  const order = await c.env.DB.prepare(
    `SELECT id FROM orders WHERE order_number = ? AND customer_email = ? AND status NOT IN ('PENDING_PAYMENT', 'CANCELLED')`
  )
    .bind(review.order_number, session.email)
    .first<{ id: string }>();
  if (!order) return c.json({ success: false, error: 'That review is not for one of your orders' }, 403);
