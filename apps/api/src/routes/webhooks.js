...
webhooksApp.post('/stripe', async (c) => {
    ...
    const existingEvent = await c.env.DB.prepare('SELECT id FROM webhook_events WHERE id = ?').bind(eventId).first();
    if (existingEvent) {
    ...
    if (eventType === 'checkout.session.completed' || eventType === 'payment_intent.succeeded') {
        ...
        if (orderId) {
          ...
          `).bind(paymentId, orderId, paymentIntentId, amountCents || order.total_cents, order.currency || 'usd', eventId),
                ...
                await featureHooks.onOrderPaid(c.env, { orderId, order, items: (items || []) });
