...
adminApp.post('/orders/:id/status', async (c) => {
  ...
  `).bind(body.status, body.tracking_number || null, body.carrier || null, orderId).run();
    // Audit Log
    await recordAuditLog(c.env.DB, actor || { id: 'admin', email: 'admin@dailygrind.coffee' }, 'ORDER_STATUS_UPDATE', 'orders', orderId, { status: oldOrder?.status }, { status: body.status, tracking: body.tracking_number }, c.req.header('CF-Connecting-IP'));
...
adminApp.get('/ai-summary', async (c) => {
    ...
    const { results: recentOrders } = await c.env.DB.prepare('SELECT order_number, total_cents, status FROM orders ORDER BY created_at DESC LIMIT 10').all();
    const { results: lowStock } = await c.env.DB.prepare('SELECT sku, available_stock, low_stock_threshold FROM inventory WHERE available_stock <= low_stock_threshold').all();
    const prompt = `
