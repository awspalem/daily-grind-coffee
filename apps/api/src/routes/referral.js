...
import { REFERRAL_RATES, buildShareTargets, checkReferral, getDashboard, getOrCreateCode, hashVisitor, recordVisit, } from '../services/referral';
...
referralApp.post('/validate', async (c) => {
    ...
    if (!email)
        return c.json({ success: false, error: 'Email required to check a referral code' }, 400);
...
referralApp.get('/terms', (c) => c.json({
    success: true,
    terms: {
