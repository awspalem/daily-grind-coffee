...
import { FREQUENCY_DAYS, acceptSaveOffer, addDays, addMonths, buildSaveOffer, cancelSubscription, findOwnedSubscription, getPlan, getPlanPerkBalances, listOwnedSubscriptions, listPlans, newId, pauseSubscription, projectUpcomingShipments, recordSubscriptionEvent, restorePaymentMethod, resumeSubscription, serialisePlan, serialiseSubscription, skipNextDelivery, swapCoffee, updateSubscription, } from '../services/subscriptionPlans';
...
async function requireCustomer(c) {
    ...
    if (!session)
        return { session: null, res: c.json(UNAUTHENTICATED, 401) };
 ...
 * guessing an id.
 */
async function requireOwnedSubscription(c, session) {
    const sub = await findOwnedSubscription(c.env.DB, c.req.param('id'), session);
    if (!sub)
        return { sub: null, res: c.json({ success: false, error: 'Subscription not found' }, 404) };
    return { sub, res: null };
}
 ...
 */
async function createDeliveryOrder(env, params) {
    ...
    `).bind(params.orderId, params.orderNumber, params.customerId, params.customerEmail, lineTotal, lineTotal, params.shippingAddressJson, params.notes),
        env.DB.prepare(`
    ...
    `).bind(newId('oi'), params.orderId, params.variantId, params.productName, params.variantId, params.grindType, params.unitPriceCents, params.quantity, lineTotal, params.frequency),
    ]);
    return { ok: true };
}
// ==================== Public catalog ====================
...
subscriptionsApp.post('/checkout', async (c) => {
  ...
  `).bind(String(body.variant_id || '')).first();
    if (!variant)
        return c.json({ success: false, error: 'Choose a coffee to start with' }, 400);
    const frequency = String(body.frequency || plan.default_frequency);
    if (!FREQUENCY_DAYS[frequency])
        return c.json({ success: false, error: 'Unknown delivery frequency' }, 400);
    ...
    `).bind(orderId, orderNumber, session.customerId, session.email, chargeCents, chargeCents, addressJson, `${plan.name} (annual, prepaid) subscription purchase`).run();
    }
    else {
    ...
    `).bind(subscriptionId, session.email, session.customerId, orderId, variant.id, variant.product_name, grind, frequency, quantity, variant.price_cents, 
        ...
        addDays(now, FREQUENCY_DAYS[frequency]), addressJson, plan.id, plan.term, now, addMonths(now, plan.term_months || 1)),
    ]);
...
subscriptionsApp.post('/:id/pause', async (c) => {
    ...
    if (sub.status === 'PAUSED')
        return c.json({ success: true, message: 'Already paused' });
...
subscriptionsApp.post('/:id/resume', async (c) => {
    ...
    if (!sub)
        return notFound;
    if (sub.status !== 'PAUSED')
        return c.json({ success: false, error: 'This subscription is not paused' }, 400);
...
subscriptionsApp.post('/:id/save-offer', async (c) => {
    ...
    const result = await acceptSaveOffer(c.env.DB, sub, kind);
    if (!result.ok)
        return c.json({ success: false, error: result.error }, 400);
...
subscriptionsApp.post('/:id/cancel', async (c) => {
    ...
    if (!sub)
        return notFound;
    if (sub.status === 'CANCELLED')
        return c.json({ success: true, message: 'Already cancelled' });
    const body = await readJson(c);
...
function readPlanInput(body) {
    ...
    if (body.tier !== undefined) {
        if (!TIERS.includes(String(body.tier)))
            return { ok: false, error: 'Unknown tier' };
        value.tier = String(body.tier);
    }
    if (body.term !== undefined) {
        if (!TERMS.includes(String(body.term)))
            return { ok: false, error: 'Term must be MONTHLY or ANNUAL' };
        value.term = String(body.term);
    }
    if (body.price_cents !== undefined) {
        const price = Math.trunc(Number(body.price_cents));
        if (!Number.isFinite(price) || price < 0)
            return { ok: false, error: 'Price must be a positive amount in paise' };
    ...
    if (body.discount_percent !== undefined) {
        ...
        if (!Number.isFinite(pct) || pct < 0 || pct > 60)
            return { ok: false, error: 'Discount must be between 0 and 60%' };
        value.discount_percent = pct;
    }
    if (body.default_frequency !== undefined) {
        if (!FREQUENCY_DAYS[String(body.default_frequency)])
    ...
    if (body.term_months !== undefined) {
        ...
        if (!Number.isFinite(months) || months < 1 || months > 36)
            return { ok: false, error: 'Term must be 1–36 months' };
    ...
    if (body.perks !== undefined) {
        if (!Array.isArray(body.perks))
            return { ok: false, error: 'Perks must be a list' };
        value.perks_json = JSON.stringify(body.perks.map((p) => String(p)));
    }
    if (body.entitlements !== undefined) {
        if (!Array.isArray(body.entitlements))
            return { ok: false, error: 'Entitlements must be a list' };
        ...
        value.entitlements_json = JSON.stringify(body.entitlements.map((e) => ({
            code: String(e.code), units: Math.trunc(Number(e.units)),
        })));
    }
    if (body.tagline !== undefined)
        value.tagline = body.tagline === null ? null : String(body.tagline);
    if (body.description !== undefined)
        value.description = body.description === null ? null : String(body.description);
    if (body.badge !== undefined)
        value.badge = body.badge === null ? null : String(body.badge);
    if (body.display_order !== undefined)
        value.display_order = Math.trunc(Number(body.display_order) || 0);
    if (body.is_active !== undefined)
        value.is_active = body.is_active ? 1 : 0;
...
adminPlans.post('/plans', async (c) => {
    ...
    if (existing)
        return c.json({ success: false, error: 'A plan with that slug already exists' }, 409);
    const id = newId('plan');
    const columns = ['id', 'slug', ...Object.keys(parsed.value)];
    const values = [id, slug, ...Object.values(parsed.value)];
    await c.env.DB.prepare(`INSERT INTO subscription_plans (${columns.join(', ')}) VALUES (${columns.map(() => '?').join(', ')})`).bind(...values).run();
...
adminPlans.patch('/plans/:id', async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE subscription_plans SET ${sets}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(...Object.values(parsed.value), plan.id).run();
...
adminPlans.delete('/plans/:id', async (c) => {
    ...
    if (!plan)
        return c.json({ success: false, error: 'Plan not found' }, 404);
    await c.env.DB.prepare('UPDATE subscription_plans SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?').bind(plan.id).run();
