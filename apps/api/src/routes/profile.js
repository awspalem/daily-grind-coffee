...
async function loadPreferences(db, customerId) {
    ...
    const { results: channels } = await db
        .prepare(`SELECT ch.id AS channel_id, ch.name, ch.channel_type, ch.status,
...
profileApp.get('/orders', async (c) => {
    ...
    const { results } = await c.env.DB
        .prepare(`SELECT o.id, o.order_number, o.status, o.total_cents, o.currency, o.tracking_number, o.created_at,
...
profileApp.post('/orders/:identifier/reorder', async (c) => {
    ...
    const [ownerId, ownerEmail] = orderOwnerBindings(session.customerId, session.email);
    const order = await c.env.DB
        .prepare(`SELECT o.id FROM orders o
        WHERE (o.id = ? OR o.order_number = ?)
          AND ${ORDER_OWNER_SQL}
        LIMIT 1`)
        .bind(identifier, identifier, ownerId, ownerEmail)
    ...
    if (!order)
        return c.json({ success: false, error: 'Order not found' }, 404);
    const { results } = await c.env.DB
        .prepare(`SELECT oi.variant_id, oi.product_name, oi.weight_grams, oi.grind_type, oi.quantity,
...
const ADDRESS_FIELDS = ['name', 'line1', 'line2', 'city', 'state', 'postal_code', 'country'];
...
profileApp.patch('/addresses/:id', async (c) => {
    ...
    for (const field of ADDRESS_FIELDS) {
        ...
        if (!text)
            return c.json({ success: false, error: `${field} cannot be empty` }, 400);
        sets.push(`${field} = ?`);
        binds.push(text);
    }
    if (sets.length === 0)
...
profileApp.put('/preferences', async (c) => {
    ...
    await c.env.DB
        .prepare(`INSERT INTO customer_preferences (customer_id, default_grind, default_weight_grams, brew_method)
       VALUES (?, ?, ?, ?)
    ...
    if (body.channels && typeof body.channels === 'object') {
        ...
        const { results: validChannels } = placeholders
            ? await c.env.DB
                ...
                .all()
            : { results: [] };
        const validIds = new Set((validChannels || []).map((row) => row.id));
        const rejected = [];
        const statements = entries.flatMap(([channelId, optedIn]) => {
            ...
            return [
                c.env.DB
                    .prepare(`INSERT INTO customer_channel_optins (id, customer_id, channel_id, opted_in)
             VALUES (?, ?, ?, ?)
             ON CONFLICT(customer_id, channel_id) DO UPDATE SET
               opted_in = excluded.opted_in,
               updated_at = CURRENT_TIMESTAMP`)
                    .bind('opt_' + crypto.randomUUID().replace(/-/g, '').slice(0, 16), session.customerId, channelId, optedIn ? 1 : 0),
            ];
        });
        ...
        if (statements.length > 0) {
            ...
            await c.env.DB
                .prepare(`INSERT INTO audit_log (id, actor_id, actor_email, action, entity_type, entity_id, new_value_json)
           VALUES (?, ?, ?, 'CHANNEL_OPTIN_UPDATE', 'customer', ?, ?)`)
                .bind('al_' + crypto.randomUUID().replace(/-/g, '').slice(0, 16), session.customerId, session.email, session.customerId, JSON.stringify(Object.fromEntries(entries)))
                .run().catch(() => { });
        }
    }
    return c.json({ success: true, preferences: await loadPreferences(c.env.DB, session.customerId) });
 ...
 * the fuzzy case.
 */
function scoreCatalog(products, profile) {
    ...
    return products.map((p) => {
        ...
        if (p.id === profile.top_product_id) {
            ...
            if (roastShare > 0)
                score += 0.75;
            if (roastShare > 0)
                reasons.push(`A ${String(p.roast_level).replace(/_/g, ' ').toLowerCase()} roast, like most of your shelf`);
            if (originShare > 0)
                reasons.push(`From ${p.origin_country}, an origin you keep coming back to`);
...
profileApp.get('/recommendations', async (c) => {
    ...
    const ranked = scoreCatalog(products, profile)
    ...
    return c.json({
        ...
        days_until_typical_reorder: cadence !== null && daysSince !== null ? Math.round(cadence - daysSince) : null,
    });
});
export { profileApp };
