...
async function runAdminToolCall(call, ctx) {
    ...
    if (toolName === 'get_inventory') {
        ...
        WHERE v.id = ?
      `).bind(String(toolArgs.variant_id)).first();
    ...
    if (toolName === 'propose_admin_action') {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(id, proposalToken, ctx.actor.id, ctx.actor.email, String(toolArgs.action_type), JSON.stringify(toolArgs.payload || {})).run();
...
function adminSessionTokenFor(actor) {
...
adminAgentApp.post('/chat/stream', async (c) => {
    ...
    const ctx = { env: c.env, db, ai, actor };
    return streamSSE(c, async (stream) => {
        ...
        const secondPassMessages = [
            ...fullMessages,
            ...toolResults.map((r) => ({ role: 'tool', tool_call_id: r.tool_call_id, name: r.name, content: JSON.stringify(r.result) })),
        ];
...
adminAgentApp.post('/confirm-action', async (c) => {
    ...
    const proposal = await c.env.DB.prepare(`SELECT * FROM admin_action_proposals WHERE proposal_token = ?`).bind(String(body.proposal_token)).first();
    if (!proposal) {
    ...
    WHERE proposal_token = ?
  `).bind(`${actor.email} approved: ${body.note || ''}`.trim(), body.proposal_token).run();
