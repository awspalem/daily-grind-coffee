...
import { AGENT_PUBLIC_ORIGIN, AGENT_VERSION, MCP_PROTOCOL_VERSION, PUBLIC_AGENT_TOOLS, STOREFRONT_PUBLIC_ORIGIN, } from '../services/agentToolSchema';
const agentDiscoveryApp = new Hono();
...
agentDiscoveryApp.get('/card', (c) => {
    const body = {
        ...
        description: 'Recommends specialty coffee from The Daily Roast (Bangalore) and answers brew questions. Can add to cart with user confirmation.',
...
agentDiscoveryApp.get('/manifest.json', (c) => {
    const body = {
        ...
        authorization: {
            type: 'none',
            description: 'Anonymous chat is allowed. Customer actions (cart, subscription) require an X-Customer-Session obtained via /api/customer/login.',
        },
...
agentDiscoveryApp.get('/openapi.json', (c) => {
    const body = {
        ...
        info: {
            ...
            version: AGENT_VERSION,
            description: 'Conversational barista agent for The Daily Roast, a specialty coffee roastery in Indiranagar, Bangalore. The agent recommends coffees, answers brew questions, and (with user confirmation) proposes adding items to a customer cart.',
        ...
        paths: {
            '/api/agent/chat': {
                post: {
                    ...
                    description: 'One-shot chat endpoint. The request body is either a single `message` string or a `messages` array; the response is the agent reply plus any tool calls it made and any `proposed_actions` that need user confirmation.',
                    requestBody: {
                        required: true,
                        content: {
                            'application/json': {
            ...
            '/api/agent/chat/stream': {
                post: {
                    ...
                    description: 'Streaming variant of /api/agent/chat. The response is `text/event-stream` with one of these event names: `status` (a tool is in flight), `delta` (one chunk of the reply), `done` (terminal: full reply + proposed_actions), `error` (terminal: the message was unrecoverable).',
                    requestBody: {
                        required: true,
                        content: {
                            'application/json': {
        ...
        components: {
            securitySchemes: {
                CustomerSession: {
                    ...
                    description: 'Optional. Adds personalisation (taste profile, loyalty, subscription) to Maya\'s context. Issued by /api/customer/login. The chat endpoint itself does not require it.',
                },
                Turnstile: {
                    type: 'apiKey',
                    in: 'header',
                    name: 'X-Turnstile-Token',
                    description: 'Required in production deployments. Cloudflare Turnstile bot-challenge token. The development environment bypasses this check.',
                },
            },
            schemas: {
                ChatMessage: {
                ...
                ChatRequest: {
                    ...
                    properties: {
                        ...
                        session_id: {
                            type: 'string',
                            description: 'Optional client-side session id. When provided, Maya restores the conversation history for this session before generating the next turn.',
                        },
                    },
                },
                ProposedAction: {
 ...
 */
const mcpWellKnownApp = new Hono();
/**
...
mcpWellKnownApp.get('/.well-known/mcp.json', (c) => {
    const body = {
        ...
        serverInfo: {
            name: 'the-daily-roast-maya',
            version: AGENT_VERSION,
            description: 'Maya, the AI barista for The Daily Roast (Bangalore). Recommends specialty coffee, answers brew questions, and proposes cart actions with user confirmation.',
            homepage: STOREFRONT_PUBLIC_ORIGIN + '/',
        },
        transport: {
