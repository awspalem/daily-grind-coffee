...
async function requireCustomer(c: any): Promise<CustomerSession | null> {
...
async function ownedBooking(
  ...
  if (!booking || booking.customer_id !== session.customerId) return null;
  return booking;
}

/** A slot as the storefront may see it. The meeting link is deliberately not included. */
function publicSlot(experience: ExperienceRow, slot: SlotRow) {
 ...
 */
function publicBooking(booking: BookingDetailRow) {
...
function trimNote(value: unknown, max = 1000): string | null {
...
experiencesApp.post('/bookings', async (c) => {
  ...
  if (!slotId) return c.json({ success: false, error: 'slotId is required' }, 400);

  const slot = await c.env.DB.prepare('SELECT * FROM experience_slots WHERE id = ?').bind(slotId).first<SlotRow>();
  ...
  const customer = await c.env.DB
    ...
    .first<{ full_name: string | null; phone: string | null }>();
...
experiencesApp.post('/bookings/:id/reschedule', async (c) => {
  ...
  const newSlot = await c.env.DB
    ...
    .first<SlotRow>();
  if (!newSlot) return c.json({ success: false, error: 'Slot not found' }, 404);
...
experiencesApp.get('/bookings/:id/calendar.ics', async (c) => {
  ...
  if (!booking) return c.text('Not found', 404);
...
const adminRoutes = new Hono<{ Bindings: Env; Variables: { adminActor: AdminActor } }>();
...
adminRoutes.patch('/experiences/:id', async (c) => {
  ...
  if (!before) return c.json({ success: false, error: 'Experience not found' }, 404);

  const body = await c.req.json().catch(() => ({} as any));
  const editable: Record<string, 'text' | 'int'> = {
...
adminRoutes.get('/slots', async (c) => {
  ...
  const params: unknown[] = [];
  if (experienceId) { clauses.push('s.experience_id = ?'); params.push(experienceId); }
  if (c.req.query('upcoming') === '1') { clauses.push('s.ends_at >= ?'); params.push(new Date().toISOString()); }
...
adminRoutes.post('/slots', async (c) => {
  ...
  if (!experience) return c.json({ success: false, error: 'Experience not found' }, 404);

  const startsAt = new Date(String(body.startsAt || ''));
  if (isNaN(startsAt.getTime())) return c.json({ success: false, error: 'startsAt must be a valid date' }, 400);
  ...
  `).bind(
    id, experience.id, startsAt.toISOString(), endsAt.toISOString(), seatsTotal,
    body.staffName || null, body.staffEmail || null,
    experience.mode === 'VIDEO' ? (body.meetingUrl || null) : null,
    body.locationOverride || null,
    body.priceCentsOverride != null ? Number(body.priceCentsOverride) : null,
    body.notes || null
  ).run();
...
adminRoutes.patch('/slots/:id', async (c) => {
  const actor = c.get('adminActor');
  const id = c.req.param('id');
  const before = await c.env.DB.prepare('SELECT * FROM experience_slots WHERE id = ?').bind(id).first<SlotRow>();
  if (!before) return c.json({ success: false, error: 'Slot not found' }, 404);

  const body = await c.req.json().catch(() => ({} as any));
  const sets: string[] = [];
  const params: unknown[] = [];

  if (body.startsAt) { sets.push('starts_at = ?'); params.push(new Date(String(body.startsAt)).toISOString()); }
  if (body.endsAt) { sets.push('ends_at = ?'); params.push(new Date(String(body.endsAt)).toISOString()); }
  if (body.staffName !== undefined) { sets.push('staff_name = ?'); params.push(body.staffName || null); }
  if (body.staffEmail !== undefined) { sets.push('staff_email = ?'); params.push(body.staffEmail || null); }
  if (body.meetingUrl !== undefined) { sets.push('meeting_url = ?'); params.push(body.meetingUrl || null); }
  if (body.locationOverride !== undefined) { sets.push('location_override = ?'); params.push(body.locationOverride || null); }
  if (body.notes !== undefined) { sets.push('notes = ?'); params.push(body.notes || null); }
  if (body.status) { sets.push('status = ?'); params.push(String(body.status)); }
...
adminRoutes.delete('/slots/:id', async (c) => {
  const actor = c.get('adminActor');
  const id = c.req.param('id');
  const slot = await c.env.DB.prepare('SELECT * FROM experience_slots WHERE id = ?').bind(id).first<SlotRow>();
  if (!slot) return c.json({ success: false, error: 'Slot not found' }, 404);
...
adminRoutes.post('/bookings/:id/attendance', async (c) => {
  ...
  if (!ok) return c.json({ success: false, error: 'Booking not found, or not in a markable state.' }, 400);
